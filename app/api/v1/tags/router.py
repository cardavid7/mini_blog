...
def list_tags(
    ...
    order_by: str = Query(default="id", regex="^(id|name)$"),
...
def update_tag(id: int, tag: TagUpdate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
        repository = TagRepository(db)
...
def delete_tag(id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
        repository = TagRepository(db)
